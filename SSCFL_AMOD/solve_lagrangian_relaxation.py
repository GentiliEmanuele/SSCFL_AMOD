...
def solve_lagrangian_relaxation(problem_instance):
    marked_lamb = []
    ...
    while True and num_runs >= 0:
        ...
        if not better_solution_found:
            num_runs -= 1
            if num_runs == 0 and find_feasible:
                h.final_adjustment_heuristic(problem_instance, best_x, best_y, k)
        if better_solution_found or z_u / z_l <= 1 + epsilon:
            ...
            marked_lamb = []
    return z_u, best_x_feasible, best_y_feasible, z_l, best_x, best_y, lamb
